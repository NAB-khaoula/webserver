...
/*   Updated: 2022/02/09 22:19:41 by ybouddou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
Request	recvRequest(char *buffer, struct kevent event)
{
	std::string		req;
	size_t	ret;
	size_t	rd;
	...
	ret = recv(event.ident, buffer, 1024, 0);
	buffer[ret] = '\0';
	...
	{
		memset(buffer, 0, 1024 + 1);
		rd = recv(event.ident, buffer, 1024, 0);
		buffer[rd] = '\0';
		req += buffer;
		ret += rd;
		// std::cout << "rd : " << rd << " | ret : " << ret << " | " << event.data << "\n";
	}
...
void	handle_connection(t_WebServ *webserv, struct kevent event)
{
	...
	request = recvRequest(buffer, event);
